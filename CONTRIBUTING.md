# Contributing to bract

bract is an early-stage project. Contributions are welcome — but read this first.

## Philosophy

bract has a strong opinion: **agents are processes, state is files, everything is observable**. Before proposing a feature, ask: does this make the system simpler or more complex? Does it respect the Unix philosophy? If a feature requires a database, a cloud service, or a daemon that can't be understood by reading files — it probably doesn't belong in core.
...
- Example `bract.yml` configurations for real use cases
- Plugins published as separate `@losoft/bract-*` packages
...
- Inbox watcher
- Supervisor
- CLI (`bract ps`, `bract spawn`, `bract send`, etc.)
- Message format
- `bract.yml` parsing
...
📝 docs: add ADR-007 for message deduplication
...
- No external runtime dependencies in `@losoft/bract-runtime` — it must run with zero installs beyond Bun
...
- Test the filesystem — bract's core promise is filesystem observability, so tests should verify files are written correctly, not just return values
- Use `tmp` directories in tests, clean up after

## Opening issues

Use the issue templates. For bugs, include:
- bract version (`bract --version`)
- OS and architecture
