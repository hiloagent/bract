# ADR-008: Pipe engine — wiring agent outboxes to inboxes
...
bract.yml already supports declaring pipes:
...
A pipe tracking file at `$BRACT_HOME/pipes/<from>-<to>.jsonl` records all
...
```sh
# Wire two agents together (adds to runtime pipe table, not bract.yml)
bract pipe researcher writer

# Wire with a filter
bract pipe researcher writer --filter '.priority == "high"'

# Disconnect
bract unpipe researcher writer

# List active pipes
bract pipes
```

Dynamic pipes are stored in `$BRACT_HOME/pipes/active.json`:
...
bract read researcher --follow | bract send writer --stdin
```
...
The `bract pipes` command shows live counts without a daemon query.
...
**Polling latency.** The inbox watcher uses polling (ADR-002). On Linux, inotify
...
7 days. The `bract gc` command handles this.
...
use cases bract targets (single-machine, tens of agents), filesystem I/O is
sufficient.
...
- ADR-002: Inbox watcher via polling — the mechanism the pipe engine relies on
