# ADR-005: Plugin and extension model
...
bract's core runtime handles the essentials: filesystem layout, inbox watching,
...
Options:
1. **Plugins as npm packages** — publish `bract-plugin-*` packages
2. **Plugins as local scripts** — shell scripts or Node scripts in a `plugins/` dir
3. **Plugins as in-process hooks** — TypeScript hooks loaded from `bract.config.ts`
4. **Plugins as sidecar processes** — separate processes that communicate via the filesystem

## Decision

Plugins are **sidecar processes** that communicate with the bract runtime via the
...
- Reads from and writes to `$BRACT_HOME/` using the documented filesystem layout
- Optionally reads a plugin config section from `bract.yml`

Example: a Telegram notification plugin

```
# Runs as a separate process, watching for outbox messages to forward
bract-plugin-telegram --token $TOKEN --chat-id $CHAT_ID --watch notify-agent
```

The plugin is not managed by the bract supervisor — it is started independently,
by systemd, or alongside `bract up` via a shell wrapper. It has no special privileges.
...
while true; do
  for f in ~/.bract/agents/notify/outbox/*.msg; do
    [ -f "$f" ] || continue
    body=$(jq -r .body "$f")
...
custom tools, lifecycle interceptors), `bract.config.ts` supports in-process hooks:
...
`bract.config.ts` is loaded by the supervisor at startup. If it fails to load,
...
`bract/client` is a thin package that knows how to write message files to the
correct inbox path. It does not require the bract runtime to be running — it just
writes files.
...
A future `plugins:` section in `bract.yml` may support declaring plugins that should
be started alongside the fleet:

```yaml
# Future — not in v1
plugins:
  - name: telegram
    package: bract-plugin-telegram
    env:
...
**Good:**
- Sidecar plugins have zero coupling to the bract runtime. They can be written in any
  language, deployed independently, and updated without touching the core.
- The filesystem is the interface. A plugin written today will still work if bract's
  internals change, as long as the filesystem layout (ADR-001) is stable.
- In-process hooks cover the tight-integration cases (custom tools, model providers)
  without requiring IPC protocols.
- `bract/client` lets any script send messages to an agent with minimal code.

**Bad:**
- Sidecar plugins must be started and managed separately. There is no `bract up`-style
...
- No plugin registry or discovery mechanism in v1. Finding community plugins requires
  searching npm for `bract-plugin-*`.

## Alternatives considered

**Plugins as npm packages loaded by CLI:** Would require the bract CLI to dynamically
...
own tooling. Tempting. But `bract/client` and the lifecycle hooks are genuinely
