# ADR-003: Plugin Protocol — Tools as Executables
...
**A bract plugin is any executable in `~/.bract/plugins/` that follows the bract tool protocol.**

The protocol:
1. The runtime discovers tools by scanning `~/.bract/plugins/` for executable files/directories
...
```
~/.bract/plugins/
├── bract-search          # single executable
├── bract-browse/         # directory with multiple files
│   ├── bract-browse      # main executable (must match dir name)
│   ├── README.md
│   └── lib/
└── bract-notify          # single executable
```

### The Describe Protocol

```bash
$ bract-search describe
```
...
```bash
$ echo '{"query": "history of Unix", "num_results": 3}' | bract-search invoke
```
...
```
BRACT_AGENT_PID=7a2f
BRACT_AGENT_NAME=researcher
BRACT_AGENT_CWD=/workspace/foo
BRACT_STATE_DIR=~/.bract/agents/7a2f
```

This lets plugins write to the agent's state directory if needed (e.g., a `browser` plugin that saves screenshots to `~/.bract/agents/7a2f/artifacts/`).

### Tool Registration

Agents get access to tools in two ways:

**1. Global tools** — plugins in `~/.bract/plugins/` are available to all agents by default.

**2. Scoped tools** — at spawn time:
```bash
bract spawn --name researcher --tools search,browse
# Only search and browse are available to this agent
```

The available tools are symlinked into `~/.bract/agents/<pid>/tools/`:
```
tools/
├── search -> ~/.bract/plugins/bract-search
└── browse -> ~/.bract/plugins/bract-browse/bract-browse
```
...
A `.bract/plugins/` directory in the working directory takes precedence over global plugins. This lets projects ship their own tools:
...
```bash
$ echo '{"url": "https://example.com"}' | bract-browse invoke
```
...
**No SDK required.** You don't need to import bract to write a plugin. You don't need to know how bract works internally. You just need to handle `describe` and `invoke`.
...
**Community ecosystem.** Anyone can publish `bract-*` npm packages, PyPI packages, or single-file scripts. The convention is the contract.
...
```bash
#!/usr/bin/env bash
# bract-echo — echoes input back as a tool result
...
```python
#!/usr/bin/env python3
# bract-search — web search using DuckDuckGo
...
Require plugins to import a bract library and call `bract.register_tool(...)`. Rejected: creates language lock-in and SDK dependency. The executable protocol is universal.
