...
**Agents in bract are modeled as Unix processes.**
...
The `bract ps` command is exactly `ps` for agents.
...
**Composability is solved.** Unix pipes and redirects already compose processes. We inherit this for free. `bract run --prompt extract.md < data.csv | bract run --prompt summarize.md` works exactly as expected.

**Observability is structural.** To watch what a process is doing, you `tail -f` its stdout. To see all running processes, you run `ps`. These work the same way in bract.
...
**Tooling integrates naturally.** `grep`, `watch`, `htop`-style tools, shell scripts, cron — all of these interact with processes in ways that transfer directly to bract agents.

### Tricky

**PIDs need content-addressing.** Unix PIDs are integers assigned by the kernel; bract PIDs are short hashes derived from the agent's identity (name + model + spawn time). This makes them stable across restarts but not strictly ordered. Commands like `bract ps --sort=pid` need to sort by time, not numeric value.

**stdin/stdout semantics change.** A Unix process reads stdin once; a bract agent may read from its inbox many times over its lifetime. We resolve this by distinguishing `bract run` (ephemeral, true stdin) from `bract spawn` (persistent, inbox-based). The model is consistent but the two modes need clear documentation.

**No kernel.** Unix processes have the kernel as a mediator — it enforces scheduling, handles signals, manages memory. bract has no equivalent. We use file locks and filesystem primitives to simulate isolation, but they are advisory, not mandatory. This is acceptable for local-first use cases.

**Process table is a directory, not kernel memory.** `bract ps` reads from `~/.bract/agents/*/meta.json`. This means a crashed agent might still appear as "running." We add a heartbeat field to `meta.json` and treat stale heartbeats (>30s) as crashed. A future GC command (`bract gc`) will clean them up.
