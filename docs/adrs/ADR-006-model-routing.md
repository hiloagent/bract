...
bract agents need to call LLMs. There are several ways to do this:
...
it falls back to the `BRACT_DEFAULT_PROVIDER` environment variable (default: error).
...
Model can be set in `bract.yml` per agent and overridden at runtime:
...
jq '[.usage.inputTokens + .usage.outputTokens] | add' \
  ~/.bract/agents/researcher/logs/2026-03-25.ndjson
```

A future `bract cost` command can aggregate this.

## Consequences

**Good:**
- Single model string in `bract.yml` — no separate provider config file
- Local-first default: no config needed for Ollama, which covers the common development case
- Environment variable overrides mean the same `bract.yml` works in dev (local) and
  prod (remote) by changing env vars at deployment
- Provider interface is minimal — adding a new provider is ~50 lines of code
- Custom providers can be registered via `bract.config.ts` (ADR-005)
...
**Separate provider config section in bract.yml:** A `providers:` block defining
