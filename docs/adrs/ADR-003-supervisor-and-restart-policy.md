# ADR-003: Supervisor and restart policy
...
`bract supervisor` runs as a long-lived process. It reads agent definitions from
`$BRACT_HOME/agents/` and maintains a watch loop.

The supervisor is itself an OS process with a PID written to `$BRACT_HOME/supervisor.pid`.
...
Each agent has a restart policy, configurable in `bract.yml`:
...
An operator must manually run `bract restart {name}` to resume.
...
`bract ps` shows restart count in the status column:
```
...
A systemd unit file template is provided in `docs/examples/bract-supervisor.service`.
...
- Crash files accumulate. A future `bract gc` command should compact old crash records.

## Alternatives considered

**Using OS-level process supervision (systemd/launchd directly):**
Possible but requires root and system configuration. bract targets developer machines
